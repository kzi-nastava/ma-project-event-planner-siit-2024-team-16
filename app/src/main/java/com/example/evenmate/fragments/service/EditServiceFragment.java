...
import com.example.evenmate.databinding.FragmentEditServiceBinding;
...
public class EditServiceFragment extends Fragment {
    ...
    private void getServiceFromArgs() {
        Bundle args = getArguments();
        if (args != null) {
            if (args.containsKey("service")) {
                currentService = (Service) args.getSerializable("service");
            }
        }
    }
    ...
    private void setupUI() {
        binding.saveButton.setOnClickListener(v -> submitForm());
    ...
    private void populateFields(Service service) {
        binding.editCategoryName.setText(service.getCategory().getName());
        binding.nameInput.setText(service.getName());
        binding.priceInput.setText(String.valueOf(service.getPrice()));
        binding.discountInput.setText(String.valueOf(service.getDiscount()));
        binding.descriptionInput.setText(service.getDescription());
        binding.distinctivenessInput.setText(service.getDistinctiveness());
        binding.reservationDeadlineInput.setText(String.valueOf(service.getReservationDeadline()));
        binding.cancellationDeadlineInput.setText(String.valueOf(service.getCancellationDeadline()));
        binding.visibilitySwitch.setChecked(service.getIsVisible());
        binding.availabilitySwitch.setChecked(service.getIsAvailable());
        if ("Automatic".equals(service.getReservationConfirmation())) {
            binding.reservationTypeRadioGroup.check(R.id.automaticRadio);
        } else {
            binding.reservationTypeRadioGroup.check(R.id.manualRadio);
        }
        if (service.getLength() != null) {
            binding.durationTypeRadioGroup.check(R.id.fixedDurationRadio);
            binding.fixedDurationLayout.setVisibility(View.VISIBLE);
            binding.rangeDurationLayout.setVisibility(View.GONE);
            binding.fixedHoursInput.setText(String.valueOf(service.getLength() / 60));
            binding.fixedMinutesInput.setText(String.valueOf(service.getLength() % 60));
        } else {
            binding.durationTypeRadioGroup.check(R.id.rangeDurationRadio);
            binding.fixedDurationLayout.setVisibility(View.GONE);
            binding.rangeDurationLayout.setVisibility(View.VISIBLE);
            binding.minHoursInput.setText(String.valueOf(service.getMinLength() / 60));
            binding.minMinutesInput.setText(String.valueOf(service.getMinLength() % 60));
            binding.maxHoursInput.setText(String.valueOf(service.getMaxLength() / 60));
            binding.maxMinutesInput.setText(String.valueOf(service.getMaxLength() % 60));
        }
    }
    ...
    private void submitForm() {
        ...
        ServiceUpdate service = new ServiceUpdate();
        ...
        viewModel.updateService(currentService.getId(), service);
    }
